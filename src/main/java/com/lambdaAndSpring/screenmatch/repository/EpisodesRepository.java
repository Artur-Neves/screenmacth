...
public interface EpisodesRepository extends JpaRepository<Episodes, Long> {
	...
	@Query("SELECT e FROM Episodes e WHERE e.season.serie.id = :id AND e.avaliation IS NOT NULL ORDER BY e.avaliation DESC LIMIT 5")
	List<Episodes> listTop5Episodes(Long id);
